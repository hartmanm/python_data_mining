...
if(dist.cluster_distances.at(dis) < min_distance ){
   output="";
output.append("dist.cluster_distances.at(dis): ");
output.append(to_string(dist.cluster_distances.at(dis)));
output.append(" min_distance: ");
output.append(to_string(min_distance));
...
// recalculate the centroid for the growing cluster
int final_cluster_size=clusters.at(last_centroid_index).cluster_points_x.size();
float x_total=0;
float y_total=0;
for(int i=0;i<final_cluster_size;i++){
x_total+=clusters.at(last_centroid_index).cluster_points_x.at(i);
y_total+=clusters.at(last_centroid_index).cluster_points_y.at(i);
output="";
output.append("final_cluster_size: ");
output.append(to_string(final_cluster_size));
output.append(" x_total: ");
...
clusters.at(last_centroid_index).centroid_x=x_total/final_cluster_size;
clusters.at(last_centroid_index).centroid_y=y_total/final_cluster_size;
