...
if(init){
if(distance <= min_distance){
//if(dist.cluster_merge_candidate != i && dist.j_index != j){
...
Output(output,false);
   ...
   if(init){dist.max_cluster_distances.push_back(min_distance);
   dist.cluster_merge_candidate.push_back(cluster_merge_candidate);
dist.j_index.push_back(j_index);
dist.distance_id.push_back(dist_iterator);
dist_iterator++;
...
float result=0; 
float smallest_max=99999;

float smallest_min=9999;

//if(!init){
//int size=dist.max_cluster_distances.size();
//for (auto & i : dist.distance_id) { 

for (auto & ja : dist.distance_id) { 
//for(int j=size;size>0;j--){
//for(int i=size;size>0;j--){


//if(dist.cluster_merge_candidate){

//if(!found_max){

float this_distance = dist.max_cluster_distances.at(ja);


float id=ja;
//float this_distance = ja;
...
//if(! init){
bool flip=false;
if(this_distance < smallest_min && this_distance > 0){
   
//if(cc>cluster_count+10)   { 
    /*
if(dist.cluster_merge_candidate.at(ja) == last_cmc && dist.j_index.at(ja) == last_ji){
   //if (this_distance < last_distance ){
output="";
output.append("id: ");
output.append(to_string(id));
output.append(" this_distance: ");
output.append(to_string(this_distance));
output.append(" cluster_merge_candidate: ");
output.append(to_string(dist.cluster_merge_candidate.at(ja)));
output.append(" j_index: ");
output.append(to_string(dist.j_index.at(ja)));
Output(output,true);
//smallest_min=last_distance;
...
last_distance = dist.max_cluster_distances.at(ja);
last_cmc=dist.cluster_merge_candidate.at(ja);
last_ji=dist.j_index.at(ja);
*/
if(! flip){
//Output_all_distances(dist);
//if(dist.cluster_merge_candidate.at(ja) != last_cmc && dist.j_index.at(ja) != last_ji){
smallest_min=this_distance;
cluster_merge_candidate=dist.cluster_merge_candidate.at(ja);
j_index=dist.j_index.at(ja);
//}
}
...
for (auto & ia : dist.cluster_merge_candidate) { 
float i_this_distance = dist.max_cluster_distances.at(ia);
//float i_this_distance = ia;
if (ia!=ja){

   output="";
output.append(" at i: ");
output.append(to_string(ia));
output.append(" this_distance: ");
output.append(to_string(this_distance));
output.append(" i_this_distance: ");
output.append(to_string(i_this_distance));
//output.append("\n");
Output(output,false);
//if(smallest_max > i_this_distance+this_distance && smallest_min < this_distance+i_this_distance){

if(smallest_min < i_this_distance){
smallest_min=i_this_distance;
} // if(smallest_max < js){


for (auto & js : i_this_distance) { 
if(smallest_min < js){
smallest_min=js;
} // if(smallest_max < js){
//} //for (auto & js : this_distance) { 
...
//output.append("\n");
Output(output,false);
cluster_merge_candidate=dist.cluster_merge_candidate.at(ia);
j_index=dist.j_index.at(ja);
...
output="";
output.append(to_string(jj));
output.append(" smallest_min: ");
output.append(to_string(smallest_min));
output.append(" cluster_merge_candidate: ");
output.append(to_string(cluster_merge_candidate));
output.append(" j_index: ");
...
/*
cc=0;
for(auto itr : clusters.cluster_id){
cc++;
} // if(itr.is_valid){
*/
...
//float distance=Euclidean(x, x2, y, y2);
